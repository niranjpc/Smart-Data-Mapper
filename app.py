...
def main():
    ...
    with st.sidebar:
        ...
        show_preview = False  # XML preview not required
    ...
    st.markdown('<div class="step-header">📁 Step 1: Upload Practitioner Data</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="info-box">
        <h4>🧠 Smart Upload</h4>
        <p>Upload your practitioner data file in CSV or Excel format. The AI will automatically detect columns and map to the required XML structure.</p>
        <p><strong>No specific column names required!</strong></p>
    </div>
    """, unsafe_allow_html=True)
    data_file = st.file_uploader(
        "Practitioner Data File", 
        type=Config.SUPPORTED_FORMATS, 
        key="practitioner_data",
        help="Practitioner data to be transformed into XML format"
    )
    if not data_file:
        st.markdown("""
        <div class="warning-box">
            <h4>⚠️ File Required</h4>
            <p>Please upload a practitioner data file to continue.</p>
        </div>
        """, unsafe_allow_html=True)
        st.stop()
    try:
        with st.spinner("🔄 Loading and analyzing file..."):
            ...
            if len(data_df) == 0 or len(data_df.columns) == 0:
                st.error("Practitioner data is empty or has no columns.")
                st.stop()
    except Exception as e:
        st.markdown(f"""
        <div class="error-box">
